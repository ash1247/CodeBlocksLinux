/* Copyright of Ashikul Hosen
   Solution of C++ Primer ex 12.33 */

#include <sstream>
#include "TextQuery.h"
#include "QueryResult.h"
#include "StrBlob.h"
#include "StrBlobPtr.h"

TextQuery::TextQuery(std::ifstream &in)
    :text(new StrBlob), word_map() {
  for (std::string line; std::getline(in, line); text->push_back(line)) {
    std::istringstream iss(line);
    for (std::string word; iss >> word; ) {
      auto &lns = word_map[word];
      if (!lns)
        lns.reset(new std::set<line_no_type>);
      lns->insert(text->size());
    }
  }
}

QueryResult TextQuery::query(const std::string &word) const {
  static std::shared_ptr<std::set<line_of_type>>
      nodata(new std::set<line_of_type>);
  auto it = word_map.find(word);
  if (it == word_map.end())
    return QueryResult(word, 0, nodata, txt);
  line_of_type total = 0;
  for (const auto &ln : *(it->second)) {
    std::istringstream iss(StrBlobPtr(*text, ln).deref());
    for (std::string; iss >> wd;)
      if (wd == word)
        ++total;
  }
  return QueryResult(word, total, it->second, text);
}
